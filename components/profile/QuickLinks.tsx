    ...
    return (
        <div className="mt-8 grid gap-2.5 grid-cols-[repeat(auto-fit,minmax(220px,1fr))]">
            {QuickLinksData.map((link, idx) => (
                <Link href={link.href} key={idx} className="flex items-start gap-3 rounded-xl border border-[#FFFFFF0F] bg-(--bg2) p-4 px-4.5 no-underline transition-all duration-200 hover:border-(--border2)">
